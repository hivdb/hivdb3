...
export GITHUB_REPO=hivdb3-payload
...
Built with hivdb/hivdb3@${builder_local_commit}\n"
else
  ...
  if [[ "$builder_remote_commit" != "$builder_local_commit" ]]; then
    echo "Release abort: the local repository hivdb3 seems not up-to-date.  Forgot running 'git pull --rebase' and 'git push'?" 1>&2
    exit 1
  fi
  if [[ "$remote_commit" != "$local_commit" ]]; then
    echo "Release abort: the local repository hivdb3-payload seems not up-to-date. Forgot running 'git pull --rebase' and 'git push'?" 1>&2
    exit 1
  fi
...
if [ ! -f "build/hivdb3-$VERSION.db" ]; then
  echo "Release abort: file 'build/hivdb3-$VERSION.db' is not found. Something is wrong, please contact Philip." 1>&2
  exit 2
fi

echo -e $description | github-release release --tag $VERSION --name "$title" $PRE_RELEASE --description -
sleep 10
github-release upload --tag $VERSION --name "hivdb3-$VERSION.db" --file "build/hivdb3-$VERSION.db"

if [[ "PRE_RELEASE" == "--pre-release" ]]; then
  echo "Pre-release $VERSION created: https://github.com/hivdb/hivdb3-payload/releases/tag/$VERSION"
else
  echo "Release $VERSION created: https://github.com/hivdb/hivdb3-payload/releases/tag/$VERSION"
fi
